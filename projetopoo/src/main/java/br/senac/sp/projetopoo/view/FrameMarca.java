			...
			public void valueChanged(ListSelectionEvent e) {
				...
				if (linha >= 0) {
					...
					if (marca.getLogo() != null) {
						try {
							ByteArrayInputStream byt = new ByteArrayInputStream(marca.getLogo());
							BufferedImage bufImg = ImageIO.read(byt);
							Image imagem = bufImg.getScaledInstance(lbLogo.getWidth(), lbLogo.getHeight(),
									Image.SCALE_SMOOTH);
							lbLogo.setIcon(new ImageIcon(imagem));
						} catch (IOException ex) {
							ex.printStackTrace();
							lbLogo.setIcon(null);
							lbLogo.setText("Erro na imagem");
						}
					} else {
						lbLogo.setIcon(null);
						lbLogo.setText("Sem imagem");
					}
				}
			}
		});
